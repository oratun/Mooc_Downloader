...
from Mooc_Config import video_fmt
...
class Icourse163_Mooc(Icourse163_Base):
    ...
    def _get_video_url(self, params):
        ...
        for index, sp in enumerate(resolutions, 1):
            video_match = re.search(r'(?P<ext>%s)%sUrl="(?P<url>.*?\.(?P=ext).*?)"' % (video_fmt, sp), text)
            if video_match:
                video_url, _ = video_match.group('url', 'ext')
                break
                # if index >= self.mode: break
        print(video_url, sub_url)
...
def main():
    # url = 'https://www.icourse163.org/course/ZJU-93001?tid=1461682474'  # 数据结构
    url = 'https://www.icourse163.org/course/NJU-1001625001?tid=1460891441'  # 计算机系统基础(一)
